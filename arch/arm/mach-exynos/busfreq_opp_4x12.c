...
static int int_bias = 0;	//add -100000 all level
module_param_named(int_bias, int_bias, int, 0644);
static int mif_bias = 0;	//add -100000 all level
module_param_named(mif_bias, mif_bias, int, 0644);
...
static unsigned int exynos4x12_timingrow[LV_END] = {
	/* 400            400                267              267              160             133              100 */
	0x34498691, 0x34498691, 0x2336544C, 0x2336544C, 0x152432C7, 0x12233246, 0x0D1321C5,
};
...
static void exynos4x12_set_bus_volt(void)
{
	...
	for (i = 0 ; i < LV_END ; i++)
		exynos4_busfreq_table[i].volt =
			exynos4_mif_volt[asv_group_index][i] + mif_bias;
...
{
	return exynos4_int_volt[asv_group_index][index] + int_bias;
}
