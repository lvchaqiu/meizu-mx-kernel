...
{
	struct clk *clk_sclk_hpm;
	struct clk *clk_copy;
	struct clk *clk_parent;
	struct clk *clk_hpm;
	struct clk *clk_iem_apc;
	struct clk *clk_iem_iec;
	
	clk_hpm = clk_get(NULL, "hpm");
	if (IS_ERR(clk_hpm)) {
		pr_err("EXYNOS4210: ASV : hpm clock get error");
		return -EINVAL;
	}
	clk_enable(clk_hpm);
	
	clk_iem_apc = clk_get(NULL, "iem_apc");
	if (IS_ERR(clk_iem_apc)) {
		pr_err("EXYNOS4210: ASV : clk_iem_apc clock get error");
		return -EINVAL;
	}
	clk_enable(clk_iem_apc);
	
	clk_iem_iec = clk_get(NULL, "iem_iec");
	if (IS_ERR(clk_iem_iec)) {
		pr_err("EXYNOS4210: ASV : clk_iem_iec clock get error");
		return -EINVAL;
	}
	clk_enable(clk_iem_iec);
	...
	clk_sclk_hpm = clk_get(NULL, "sclk_hpm");
	if (IS_ERR(clk_sclk_hpm)) {
		pr_info("EXYNOS4210: ASV: Fail to get sclk_hpm\n");
		return -EINVAL;
	}

	clk_set_rate(clk_sclk_hpm, (200 * 1000 * 1000));

	clk_put(clk_sclk_hpm);
