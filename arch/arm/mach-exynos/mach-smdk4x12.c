...
static struct max77686_platform_data __initdata exynos4_max77686_info = {
...
#endif
#if defined(CONFIG_VIDEO_SAMSUNG_S5P_MFC) || defined(CONFIG_VIDEO_MFC5X)
	sysmmu_set_owner(&SYSMMU_PLATDEV(mfc_l).dev, &s5p_device_mfc_l.dev);
	sysmmu_set_owner(&SYSMMU_PLATDEV(mfc_r).dev, &s5p_device_mfc_r.dev);
#endif
#if defined(CONFIG_VIDEO_FIMC)
...
{
#ifdef CONFIG_VIDEO_FIMC_SYSMMU
	s5p_create_iommu_mapping(&s3c_device_fimc0.dev, 0x20000000, SZ_128M, 4);
	s5p_create_iommu_mapping(&s3c_device_fimc1.dev, 0x20000000, SZ_128M, 4);
	s5p_create_iommu_mapping(&s3c_device_fimc2.dev, 0x20000000, SZ_128M, 4);
	s5p_create_iommu_mapping(&s3c_device_fimc3.dev, 0x20000000, SZ_128M, 4);
#endif	
#ifdef CONFIG_VIDEO_MFC5X_SYSMMU
