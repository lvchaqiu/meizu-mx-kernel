...
static int s5p_tvout_enable_power(int on)
{
	return 0;
}
void __init s5p_tvout_set_platdata(struct s5p_platform_tvout *tv)
{
	struct s5p_platform_tvout *tvout;

	tvout = kmemdup(tv, sizeof(struct s5p_platform_tvout), GFP_KERNEL);
	if (!tvout)
		printk(KERN_ERR "%s: no memory for platform data\n", __func__);
	else {
		if (!tvout->enable_power)
			tvout->enable_power = s5p_tvout_enable_power;

		s5p_device_tvout.dev.platform_data = tvout;
	}
}
