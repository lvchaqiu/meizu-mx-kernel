...
static struct platform_device *mx_spdif_device = NULL;

void audio_spdif_output_en(int en)
{
	struct mx_spdif_platform_data *spdif_data;
	if(mx_spdif_device) {
		spdif_data = mx_spdif_device->dev.platform_data;
		if(spdif_data && spdif_data->spdif_output_enable) {
			spdif_data->spdif_output_enable(en);
		}
	}
}
EXPORT_SYMBOL(audio_spdif_output_en);

static int __devinit mx_spdif_probe(struct platform_device *pdev)
{
	int ret;

	printk("%s++\n", __func__);
	mx_spdif_device = pdev;
...
static int __devexit mx_spdif_remove(struct platform_device *pdev)
{
...
static struct platform_driver mx_spdif_driver={
	.probe = mx_spdif_probe,
	.remove = mx_spdif_remove,	
	.driver		= {
		.name	= "mx-spdif",
		.owner	= THIS_MODULE,
	},
...
static int __init smdk_init(void)
{
	...
	ret = platform_driver_register(&mx_spdif_driver);
	if (ret)
		return -ENOMEM;
	printk("%s--\n", __func__);
	return ret;
...
{
	platform_driver_unregister(&mx_spdif_driver);
}
