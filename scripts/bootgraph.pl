...
print "<svg height=\"10000\" width=\"100%\" version=\"1.1\" xmlns=\"http://www.w3.org/2000/svg\">\n";
...
my $flag = 0;
my $mult = 10000.0/ ($maxtime - $firsttime);
...
foreach my $key (@initcalls) {
	...
	if ($duration >= $threshold) {
		my ($s, $s2, $s3, $e, $w, $y, $y2, $y3, $delta, $style);
		my $pid = $pids{$key};
		...
		if ($type{$key} == 1) {
			$y = $y + 15;
			print "<rect y=\"$s\" height=\"$w\" x=\"$y\" width=\"165\" style=\"$style_wait\"/>\n";
		} else {
			print "<rect y=\"$s\" height=\"$w\" x=\"$y\" width=\"195\" style=\"$style\"/>\n";
			if ($duration >= $threshold2) {
				print "<text transform=\"translate($y2,$s2) rotate(0)\">$key ($delta)</text>\n";
			} else {
				print "<text transform=\"translate($y3,$s3) rotate(0)\">$key ($delta)</text>\n";
			}
		}
	}
}
...
my $time = $firsttime;
my $step = ($maxtime - $firsttime) / 200;
while ($time < $maxtime) {
	my $s3 = ($time - $firsttime) * $mult;
	my $tm = int($time * 100) / 100.0;
	print "<text transform=\"translate(89,$s3) rotate(0)\">$tm</text>\n";
	$time = $time + $step;
}
