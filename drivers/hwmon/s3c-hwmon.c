...
static struct attribute *s3c_hwmon_attrs[] = {
...
*/
static int __devinit s3c_hwmon_probe(struct platform_device *pdev)
{
	struct s3c_hwmon_pdata *pdata = pdev->dev.platform_data;
	...
	hwmon->client = s3c_adc_register(pdev, NULL, NULL, 0);
	...
	ret = s3c_hwmon_add_raw(&pdev->dev);
	if (ret)
		goto err_registered;
	...
	if (IS_ERR(hwmon->hwmon_dev)) {
		dev_err(&pdev->dev, "error registering with hwmon\n");
	...
	for (i = 0; i < ARRAY_SIZE(pdata->in); i++) {
		...
		if (cfg->div == 0) {
			dev_err(&pdev->dev, "channel %d divider zero\n", i);
			continue;
		}

		ret = s3c_hwmon_create_attr(&pdev->dev, pdata->in[i],
					    &hwmon->attrs[i], i);
		if (ret) {
			dev_err(&pdev->dev,
			...
			for (i--; i >= 0; i--)
				s3c_hwmon_remove_attr(&pdev->dev,
							      &hwmon->attrs[i]);
...
static int __devexit s3c_hwmon_remove(struct platform_device *pdev)
{
	struct s3c_hwmon *hwmon = platform_get_drvdata(pdev);
	int i;

	s3c_hwmon_remove_raw(&pdev->dev);

	for (i = 0; i < ARRAY_SIZE(hwmon->attrs); i++)
		s3c_hwmon_remove_attr(&pdev->dev, &hwmon->attrs[i]);
