				   ...
				   void *data)
{
	...
	if (profile->qos_type || profile->enable_dev_pm_qos ||
		...
		for (i = 1; profile->qos_list[i].freq; i++) {
			 ...
			 * If QoS type is latency(PM_QOS_MIN)0like, qos_value
...
#if 1
/*just for test*/
static struct pm_qos_request qos_test;
static ssize_t store_qos_min_freq(struct device *dev, struct device_attribute *attr,
			      const char *buf, size_t count)
{
	struct devfreq *df = to_devfreq(dev);
	...
	ret = sscanf(buf, "%lu", &qos_value);
	if (ret != 1)
		goto out;

	/*Do not acquire lock, devfreq_qos_notifier_call
	 *  will acquire df->lock
	 */
	//mutex_lock(&df->lock);
	max = df->max_freq;
	if (qos_value && max && qos_value > max) {
		ret = -EINVAL;
		goto unlock;
	}
	
	if (qos_test.pm_qos_class == 0) {
		pm_qos_add_request(&qos_test,
				   PM_QOS_BUS_DMA_THROUGHPUT, qos_value);
	} else {
		pm_qos_update_request(&qos_test, qos_value);
	}	
...
static struct device_attribute devfreq_attrs[] = {
	...
	__ATTR(min_freq, /*S_IRUGO | S_IWUSR,*/ 0666, show_min_freq, store_min_freq),
	__ATTR(max_freq, S_IRUGO | S_IWUSR, show_max_freq, store_max_freq),
	__ATTR(qos_min_freq, /*S_IRUGO*/ 0666, show_qos_min_freq, store_qos_min_freq),
	{ },
};
