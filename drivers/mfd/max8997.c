...
static struct mfd_cell max8997_devs[] = {
	{ .name = "max8997-pmic", .id=0,},
	{ .name = "max8997-rtc", .id=1,},
	{ .name = "max8997-charger", .id=2,},
	{ .name = "max8997-haptic", .id=3,},
	{ .name = "max8997-muic", .id=4,},
	{ .name = "torch-led",.id=5,},	
};
...
static int max8997_flash_setup(struct i2c_client *i2c)
{
	int ret = 0;

	ret = max8997_write_reg(i2c, MAX8997_REG_BOOST_CNTL,
			MAX8997_FLED_BOOST_ENABLE);
	if (ret) return ret;

	ret = max8997_write_reg(i2c, MAX8997_REG_FLASH_CNTL,
		MAX8997_FLASH_TIME_800MS | MAX8997_FLASH_MAX_TIMER_MODE | MAX8997_FLASH_TIMER_RESET_DISABLE);
	if (ret) return ret;

	ret = max8997_write_reg(i2c, MAX8997_REG_LEN_CNTL, 0);
	if (ret) return ret;
	
	 /* disable GSMB function */
	ret = max8997_write_reg(i2c, MAX8997_REG_GSMB_CUR, 0);
			    ...
			    const struct i2c_device_id *id)
{
	...
	max8997_flash_setup(max8997->i2c);
