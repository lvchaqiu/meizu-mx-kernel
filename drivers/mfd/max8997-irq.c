...
static irqreturn_t max8997_irq_thread(int irq, void *data)
{
	...
	if (ret < 0) {
		dev_info(max8997->dev, "try to read interrupt source again\n");
		msleep(50);
		ret = max8997_read_reg(max8997->i2c, MAX8997_REG_INTSRC, &irq_src);
		if(ret < 0) {
...
int max8997_irq_init(struct max8997_dev *max8997)
{
	...
	do {
		 ...
		 * is always failed.
		 */
		ret = max8997_read_reg(max8997->i2c, MAX8997_REG_PMIC_ID0, &data);
		if (ret)
		...
		//clear the charger insert ana remove interrupt first
		ret = max8997_read_reg(max8997->i2c, MAX8997_REG_INT3, &data);
		if (ret)
			dev_err(max8997->dev, "%s: fail to read Interrupt Source Register(%d)\n",
					__func__, ret);
		else
			dev_info(max8997->dev, "%s: Interrupt Source Register(0x%02x)\n", __func__, data);
	} while (0);
	...
	for (i = 0; i < MAX8997_IRQ_GROUP_NR; i++) {
		...
		max8997_write_reg(i2c, max8997_mask_reg[i], max8997->irq_masks_cur[i]);
	}

	do {
		for (i = MUIC_INT1; i <= MUIC_INT3; i++) {
			max8997->irq_masks_cur[i] = 0x00;
			max8997->irq_masks_cache[i] = 0x00;
			max8997_write_reg(max8997->muic, max8997_mask_reg[i], max8997->irq_masks_cur[i]);
		}
	} while(0);
