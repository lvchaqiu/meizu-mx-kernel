...
#ifdef BCMEMBEDIMAGE
#include <fw_bcm4329.h>
#include <fw_bcm4329_apsta.h>
#include <fw_bcm4329_mfgtest.h>
#include <nvram_4329.h>
#endif /* BCMEMBEDIMAGE */
...
/*for wifi test firmware*/
static int test_firmware = 0; // 1 => test firmware, 0 => normal firmware
void need_test_firmware(void)
{
	test_firmware = 1;
}

void reset_firmware_type(void)
{
	test_firmware = 0;
}
...
#ifdef BCMEMBEDIMAGE
static int dhdsdio_download_code_array(struct dhd_bus *bus, uint8* dlarray, int len);
#endif
...
#ifdef BCMEMBEDIMAGE
static int
dhdsdio_download_code_array(struct dhd_bus *bus, uint8* dlarray, int len)
{
	int bcmerror = -1;
	int offset = 0;
...
_dhdsdio_download_firmware(struct dhd_bus *bus)
{
	...
	if (embed) {
		...
		if (strstr(bus->fw_path, "apsta") != NULL) {
			dlarray = (uint8 *)fw_bcm4329_apsta;
			array_len = sizeof(fw_bcm4329_apsta);
			printk("download form fw_bcm4329_apsta array\n");
		} else {
			if(test_firmware) {
				dlarray = (uint8 *)fw_bcm4329_test;
				array_len = sizeof(fw_bcm4329_test);
			} else {
				dlarray = (uint8 *)fw_bcm4329;
				array_len = sizeof(fw_bcm4329);
			}
			printk("download form fw_bcm4329 array\n");
		}
#endif
		if (dhdsdio_download_code_array(bus, dlarray, array_len)) {
	...
	do {
		...
		dhd_bus_set_nvram_params(bus, nvram_array);		
		bus->nv_path = NULL;
	} while(0);
#endif
