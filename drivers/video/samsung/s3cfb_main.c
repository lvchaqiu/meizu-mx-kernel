...
static void set_lcd_freq(struct work_struct *work)
{
	struct s3cfb_global *fbdev =
		list_entry(work, struct s3cfb_global, dfs_work.work);
...
static int lcd_dfs_notify(struct notifier_block *nb,
			  unsigned long action, void *data)
{
	struct s3cfb_global *fbdev =
		list_entry(nb, struct s3cfb_global, dfs_nb);
#ifdef CONFIG_EXYNOS_DEV_PD
	struct platform_device *pdev = to_platform_device(fbdev->dev);
...
static int s3cfb_probe(struct platform_device *pdev)
{
	...
	if (machine_is_m031() || machine_is_m032()) {
		INIT_DELAYED_WORK(&fbdev[0]->dfs_work, set_lcd_freq);
		fbdev[0]->dfs_nb.notifier_call = lcd_dfs_notify;
		fb_register_client(&fbdev[0]->dfs_nb);
	}
#endif
...
#ifdef CONFIG_FB_DYNAMIC_FREQ
	if (machine_is_m031() || machine_is_m032()) {
		fb_unregister_client(&fbdev[0]->dfs_nb);
	}
#endif
...
void s3cfb_early_suspend(struct early_suspend *h)
{
	...
	if (machine_is_m031() || machine_is_m032()) {
		fbdev[0] = fbfimd->fbdev[0];
		flush_delayed_work_sync(&fbdev[0]->dfs_work);
	}
#endif
