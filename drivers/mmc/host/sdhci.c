...
static void sdhci_request(struct mmc_host *mmc, struct mmc_request *mrq)
{
	...
	if (host->mmc->caps2 & MMC_CAP2_CLOCK_GATING) {
		...
		if(unlikely(!host->clock)) {
			sdhci_set_clock(host, host->clk_restore);
		}
		queue_delayed_work(system_nrt_wq, &host->gate_dwork,
			msecs_to_jiffies(CLK_GATE_TIMEOUT));
	}
#endif
...
{
	struct sdhci_host *host = list_entry(work, struct sdhci_host, gate_dwork.work);
	unsigned long flags;
	unsigned tmp;
	...
	tmp = readl(host->ioaddr + SDHCI_PRESENT_STATE) & mask;
	if (likely(tmp || host->mrq)) {
		queue_delayed_work(system_nrt_wq, &host->gate_dwork, 
			msecs_to_jiffies(CLK_GATE_TIMEOUT));
	} else {
		if(host->clock)
