...
#ifdef CONFIG_MMC_MSHCI_S3C_DMA_MAP
	.dma_map_sg_mshci	= mshci_s3c_dma_map_sg,
	.dma_unmap_sg_mshci	= mshci_s3c_dma_unmap_sg,
#endif
...
static int __devinit mshci_s3c_probe(struct platform_device *pdev)
{
	...
	host->vmmc = regulator_get(dev, "iNAND");
	if (IS_ERR(host->vmmc)) {
		printk(KERN_INFO "no vmmc regulator found\n");
		host->vmmc = NULL;
	} else {
...
static int mshci_s3c_suspend(struct platform_device *pdev, pm_message_t pm)
{
	struct mshci_host *host = platform_get_drvdata(pdev);
	int ret;

	ret = mshci_suspend_host(host, pm);
	WARN_ON(ret);

	return 0;
}
static int mshci_s3c_resume(struct platform_device *pdev)
{
	struct mshci_host *host = platform_get_drvdata(pdev);
