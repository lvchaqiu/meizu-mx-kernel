...
mali_io_address clk_orion_register_map=0;
...
int mali_orion_regulator_get_usecount(void)
{
	...
	{
		MALI_DEBUG_PRINT(1, ("error on mali_orion_regulator_get_usecount : g3d_regulator is null\n"));
		return 0;
	}
...
void mali_orion_regulator_disable(void)
{
	...
	if( IS_ERR_OR_NULL(g3d_regulator) )
	{
		MALI_DEBUG_PRINT(1, ("error on mali_orion_regulator_disable : g3d_regulator is null\n"));
		return;
	}
...
void mali_orion_regulator_enable(void)
{
	...
	if( IS_ERR_OR_NULL(g3d_regulator) )
	{
		MALI_DEBUG_PRINT(1, ("error on mali_orion_regulator_enable : g3d_regulator is null\n"));
		return;
	}
...
void mali_orion_regulator_set_voltage(int min_uV, int max_uV)
{
	...
	if( IS_ERR_OR_NULL(g3d_regulator) )
	{
		MALI_DEBUG_PRINT(1, ("error on mali_orion_regulator_set_voltage : g3d_regulator is null\n"));
		return;
	}
...
mali_bool mali_orion_clk_get(mali_bool bis_vpll)
{
...
void mali_orion_clk_put(mali_bool binc_mali_clock)
{
...
static mali_bool init_mali_orion_clock(void)
{
	...
	if (mali_orion_clk_set_rate(mali_gpu_clk, GPU_MHZ) == MALI_FALSE)
	{
		ret = MALI_FALSE;
		goto err_clock_get;
	}

	MALI_PRINT(("init_mali_orion_clock mali_clock %p \n", mali_clock));
	...
	regulator_enable(g3d_regulator);
	MALI_DEBUG_PRINT(1, ("= regulator_enable -> use cnt: %d \n",mali_orion_regulator_get_usecount()));
	mali_orion_regulator_set_voltage(mali_gpu_vol, mali_gpu_vol);
#endif
...
err_clock_get:
	mali_orion_clk_put(MALI_TRUE);

	return ret;
}

static mali_bool deinit_mali_orion_clock(void)
{
	if (mali_clock == 0)
		return MALI_TRUE;
...
static _mali_osk_errcode_t enable_mali_orion_clocks(void)
{
	int err;
	...
	// set clock rate
	mali_orion_clk_set_rate(mali_gpu_clk, GPU_MHZ);
	
	MALI_SUCCESS;
}

static _mali_osk_errcode_t disable_mali_orion_clocks(void)
{
...
_mali_osk_errcode_t mali_orion_platform_init()
{
	MALI_CHECK(init_mali_orion_clock(), _MALI_OSK_ERR_FAULT);

	mali_gpu_clk   = 160;
	mali_gpu_vol  = 1100000;        /* 1.10V for ASV */
#if MALI_DVFS_ENABLED
	if (!clk_orion_register_map) clk_orion_register_map = _mali_osk_mem_mapioregion( CLK_DIV_STAT_G3D, 0x20, CLK_DESC );
	if(!init_mali_orion_dvfs_status(MALI_DVFS_DEFAULT_STEP))
...
_mali_osk_errcode_t mali_orion_platform_deinit()
{
	deinit_mali_orion_clock();

#if MALI_DVFS_ENABLED
	deinit_mali_orion_dvfs_status();
	if (clk_orion_register_map )
	{
		_mali_osk_mem_unmapioregion(CLK_DIV_STAT_G3D, 0x20, clk_orion_register_map);
		clk_orion_register_map=0;
	}
#endif
...
{
	MALI_DEBUG_PRINT(3,("power down is called in mali_orion_platform_powerdown state %x core %x \n", gpu_power_state, cores));
	...
	if (gpu_power_state != 0) // power down after state is 0
	{
		...
		if (gpu_power_state == 0)
		{
			...
			disable_mali_orion_clocks();			
		}
	}
	else
	{
		MALI_PRINT(("mali_orion_platform_powerdown gpu_power_state == 0 and cores %x \n", cores));
	}
...
{
	MALI_DEBUG_PRINT(3,("power up is called in mali_orion_platform_powerup state %x core %x \n", gpu_power_state, cores));
...
void mali_orion_gpu_utilization_handler(u32 utilization)
{
	...
	{
#if MALI_DVFS_ENABLED
		if(!mali_orion_dvfs_handler(utilization))
