...
struct acm_share{
	...
	spinlock_t			ch_lock[ACM_INTERFACES];	
	acm_wait_state_t		ch_state[ACM_INTERFACES];
	wait_queue_head_t		ch_wq[ACM_INTERFACES];
	...
	unsigned int  usb_auto_suspend;

	struct workqueue_struct *post_resume_wq;
	struct work_struct post_resume_work;/*work to resume from dpm*/
