...
struct smd_shared_v1 {
	...
	struct smd_half_channel ch2;
	unsigned char data2[SMD_BUF_SIZE];
	struct smd_half_channel ch3;
	unsigned char data3[SMD_BUF_SIZE];
};
...
typedef int (*rx_complete_cb_t)(void*, // cid
					char *, // rx_addr
					int, 	// rx_len
					int, 	// rx_flags
					int *	// free
...
struct acm_ch {
	...
	int (*rx_cb)(void*, char *, int, int, int *);	
	void (*wr_cb)(void*);
	
	int (*open)(struct acm_ch *ch);
	void (*close)(struct acm_ch *ch);
	int (*is_ready)(struct acm_ch *ch);
	int (*get_write_frame)(struct acm_ch *ch);
	int (*get_write_room)(struct acm_ch *ch);
...
struct smd_channel {
	...
	volatile struct smd_half_channel *chuck;	
	unsigned char *send_data;
	unsigned char *recv_data;
	unsigned char *chuck_data;
...
#define SMD_EVENT_CONNECT 		4
#define SMD_EVENT_DISCONNECT 	5
#define SMD_EVENT_SUSPEND 		6
#define SMD_EVENT_RESUME 		7
