...
static struct jpeg_fmt formats_v1[] = {
	{
		.name		= "JPEG compressed format 422",
		...
		.name		= "JPEG compressed format 420",
...
static struct jpeg_fmt formats_v2[] = {
	{
...
static struct jpeg_fmt *find_format_v1(struct v4l2_format *f)
{
	struct jpeg_fmt *fmt;
	unsigned int i;

	for (i = 0; i < ARRAY_SIZE(formats_v1); ++i) {
		fmt = &formats_v1[i];
		if (fmt->fourcc == f->fmt.pix_mp.pixelformat)
			break;
	}

	return (i == ARRAY_SIZE(formats_v1)) ? NULL : fmt;
}

static struct jpeg_fmt *find_format_v2(struct v4l2_format *f)
{
	struct jpeg_fmt *fmt;
	unsigned int i;

	for (i = 0; i < ARRAY_SIZE(formats_v2); ++i) {
		fmt = &formats_v2[i];
		if (fmt->fourcc == f->fmt.pix_mp.pixelformat)
			break;
	}

	return (i == ARRAY_SIZE(formats_v2)) ? NULL : fmt;
}
				...
				struct v4l2_fmtdesc *f)
{
	...
	if ((ctx->dev->cpu_type == TYPE_EXYNOS4210 &&
		f->index >= ARRAY_SIZE(formats_v1)) ||
		(ctx->dev->cpu_type == TYPE_EXYNOS4x12 &&
			f->index >= ARRAY_SIZE(formats_v2)))
		return -EINVAL;

	fmt = ctx->dev->cpu_type == TYPE_EXYNOS4210 ?
		&formats_v1[f->index] : &formats_v2[f->index];
			       ...
			       struct v4l2_format *f)
{
	...
	if (ctx->dev->cpu_type == TYPE_EXYNOS4210)
		fmt = find_format_v1(f);
	else
		fmt = find_format_v2(f);
			    ...
			    struct v4l2_control *ctrl)
{
	...
	case V4L2_CID_CAM_JPEG_ENCODEDSIZE:
		if (dev->cpu_type == TYPE_EXYNOS4210)
			...
			ctrl->value = jpegv2_get_stream_size(dev->reg_base);
		break;
	default:
		break;
	}
