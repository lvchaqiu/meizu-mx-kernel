/* linux/drivers/media/video/samsung/jpeg_mx/jpeg_dev.c
 *
...
static void jpeg_device_enc_run(void *priv)
{
	...
	if (dev->cpu_type == TYPE_EXYNOS4210) {
		jpeg_sw_reset(dev->reg_base);
		jpeg_set_clk_power_on(dev->reg_base);
		jpeg_set_mode(dev->reg_base, 0);
		jpeg_set_enc_in_fmt(dev->reg_base, enc_param.in_fmt);
		jpeg_set_enc_out_fmt(dev->reg_base, enc_param.out_fmt);
		jpeg_set_enc_dri(dev->reg_base, 2);
		jpeg_set_frame_size(dev->reg_base,
			enc_param.in_width, enc_param.in_height);
		vb = v4l2_m2m_next_dst_buf(ctx->m2m_ctx);
		jpeg_set_stream_addr(dev->reg_base, dev->vb2->plane_addr(vb, 0));
		vb = v4l2_m2m_next_src_buf(ctx->m2m_ctx);
		jpeg_set_frame_addr(dev->reg_base, dev->vb2->plane_addr(vb, 0));
		jpeg_set_enc_coef(dev->reg_base);
		jpeg_set_enc_qtbl(dev->reg_base, enc_param.quality);
		jpeg_set_enc_htbl(dev->reg_base);
		jpeg_start_encode(dev->reg_base);
	}else {
		jpegv2_sw_reset(dev->reg_base);
		jpegv2_set_interrupt(dev->reg_base);
		jpegv2_set_huf_table_enable(dev->reg_base, 1);
		jpegv2_set_enc_tbl(dev->reg_base);
		jpegv2_set_encode_tbl_select(dev->reg_base, enc_param.quality);
		jpegv2_set_stream_size(dev->reg_base,
			enc_param.in_width, enc_param.in_height);
		jpegv2_set_enc_out_fmt(dev->reg_base, enc_param.out_fmt);
		jpegv2_set_enc_in_fmt(dev->reg_base, enc_param.in_fmt);
		vb = v4l2_m2m_next_dst_buf(ctx->m2m_ctx);
		...
		jpegv2_set_encode_hoff_cnt(dev->reg_base, enc_param.out_fmt);

		jpegv2_set_enc_dec_mode(dev->reg_base, ENCODING);
	}
...
static void jpeg_device_dec_run(void *priv)
{
	...
	if (dev->cpu_type == TYPE_EXYNOS4210) {
		...
		vb = v4l2_m2m_next_dst_buf(ctx->m2m_ctx);
		jpeg_set_frame_addr(dev->reg_base, dev->vb2->plane_addr(vb, 0));
		jpeg_start_decode(dev->reg_base);

	}else{
		jpegv2_sw_reset(dev->reg_base);
		jpegv2_set_interrupt(dev->reg_base);

		jpegv2_set_encode_tbl_select(dev->reg_base, 0);
		...
		if (dec_param.out_width > 0 && dec_param.out_height > 0) {
			if ((dec_param.out_width * 2 == dec_param.in_width) &&
				...
				jpegv2_set_dec_scaling(dev->reg_base, JPEG_SCALE_2, JPEG_SCALE_2);
			else if ((dec_param.out_width * 4 == dec_param.in_width) &&
				(dec_param.out_height * 4 == dec_param.in_height))
				jpegv2_set_dec_scaling(dev->reg_base, JPEG_SCALE_4, JPEG_SCALE_4);
			else
				jpegv2_set_dec_scaling(dev->reg_base, JPEG_SCALE_NORMAL, JPEG_SCALE_NORMAL);
		}

		jpegv2_set_dec_out_fmt(dev->reg_base, dec_param.out_fmt);
		jpegv2_set_dec_bitstream_size(dev->reg_base, dec_param.size);
...
void jpeg_v1_int_pending(struct jpeg_dev *ctrl)
{
...
void jpeg_v2_int_pending(struct jpeg_dev *ctrl)
{
	unsigned int	int_status;

	int_status = jpegv2_get_int_status(ctrl->reg_base);
	jpeg_dbg("state(%d)\n", int_status);
...
static void jpeg_int_pending(struct jpeg_dev *ctrl)
{
	if (ctrl->cpu_type == TYPE_EXYNOS4210)
		jpeg_v1_int_pending(ctrl);
	else /* default 4x12 */
		jpeg_v2_int_pending(ctrl);
}
...
static irqreturn_t jpeg_irq(int irq, void *priv)
{
	...
	if (ctx == 0) {
		...
		jpeg_int_pending(ctrl);
		if (ctrl->cpu_type == TYPE_EXYNOS4210)
			jpeg_sw_reset(ctrl->reg_base);
		else /* default 4x12 */
			jpegv2_sw_reset(ctrl->reg_base);
...
static int jpeg_probe(struct platform_device *pdev)
{
	...
	dev->plat_dev = pdev;
	dev->cpu_type = platform_get_device_id(pdev)->driver_data;
...
static int jpeg_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	if (dev->cpu_type == TYPE_EXYNOS4x12) {
#ifdef CONFIG_PM_RUNTIME
		pm_runtime_put_sync(&pdev->dev);
#endif
	}
...
static int jpeg_suspend_pd(struct device *dev)
{
...
static int jpeg_resume_pd(struct device *dev)
{
	struct platform_device *pdev;
	int ret;
...
static int jpeg_runtime_suspend(struct device *dev)
{
	...
	if (jpeg_drv->cpu_type == TYPE_EXYNOS4210)
		return 0;
#ifdef CONFIG_BUSFREQ_OPP
	/* lock bus frequency */
...
static struct platform_device_id s5p_jpeg_driver_ids[] = {
	{
		.name		= "jpeg_v1",
		.driver_data	= TYPE_EXYNOS4210,
	}, {
		.name		= "jpeg_v2",
		.driver_data	= TYPE_EXYNOS4x12,
	},
	{ }
};
MODULE_DEVICE_TABLE(platform, s5p_jpeg_driver_ids);


static struct platform_driver s5p_jpeg_driver = {
	.id_table	= s5p_jpeg_driver_ids,
	.probe		= jpeg_probe,
	.remove		= jpeg_remove,
...
static int __init jpeg_init(void)
{
	...
	platform_driver_register(&s5p_jpeg_driver);
...
MODULE_DESCRIPTION("JPEG mx Device Driver");
MODULE_LICENSE("GPL");
