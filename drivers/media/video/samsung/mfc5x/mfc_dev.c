...
static int __devinit mfc_probe(struct platform_device *pdev)
{
	...
	mfcdev->mem_dev_l = device_find_child(&pdev->dev, "s5p-mfc-l",
					   match_child);
	if (!mfcdev->mem_dev_l) {
	...
	pr_info("mfcdev->mem_dev_l = 0x%p\n", mfcdev->mem_dev_l);

	mfcdev->mem_dev_r = device_find_child(&pdev->dev, "s5p-mfc-r",
					   match_child);
	if (!mfcdev->mem_dev_r) {
