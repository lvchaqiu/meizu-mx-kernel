...
static struct s5p_hdmi_v_format_m03x s5p_hdmi_v_fmt_m03x[] = {
	[v720x480p_60Hz] = {
		.frame = {
			.vH_Line = 0x035a,
			.vV_Line = 0x020d,
...
// #else
static struct s5p_hdmi_v_format_m030 s5p_hdmi_v_fmt_m030[] = {
	[v720x480p_60Hz] = {
		.frame = {
			.vic		= 2,
			.vic_16_9	= 3,
	...
	struct s5p_hdmi_video *video, u8 *avi)
{
	...
	struct s5p_hdmi_v_frame_m030 frame_m030;
	struct s5p_hdmi_v_frame_m03x frame_m03x;

	memset(&frame_m030, 0, sizeof(struct s5p_hdmi_v_frame_m030));
	memset(&frame_m03x, 0, sizeof(struct s5p_hdmi_v_frame_m03x));
	if(soc_is_exynos4210()) {
		frame_m030 = s5p_hdmi_v_fmt_m030[mode].frame;
	} else {
		frame_m03x = s5p_hdmi_v_fmt_m03x[mode].frame;
	}
	...
	if(soc_is_exynos4210()) {
		avi[3] = (video->aspect == HDMI_PIC_RATIO_16_9) ?
					frame_m030.vic_16_9 : frame_m030.vic;
	} else {
		avi[3] = (video->aspect == HDMI_PIC_RATIO_16_9) ?
					frame_m03x.vAVI_VIC_16_9 : frame_m03x.vAVI_VIC;
	}
// #else
#endif
	if(soc_is_exynos4210()) {
		avi[4] = frame_m030.repetition;
	} else {
		avi[4] = frame_m03x.repetition;
	}
}
...
#ifdef CONFIG_HDMI_14A_3D
	if(soc_is_exynos4210()) {
		s5p_hdmi_reg_bluescreen_clr_m030(bl->b, bl->g, bl->r);
	} else {
		s5p_hdmi_reg_bluescreen_clr_m03x(bl->b, bl->g, bl->r);
	}
// #else
#endif
...
#ifdef CONFIG_HDMI_14A_3D
	if(soc_is_exynos4210()) {
		s5p_hdmi_reg_gcp(s5p_hdmi_v_fmt_m030[mode].frame.i_p, packet->gcp);
	} else {
		s5p_hdmi_reg_gcp(s5p_hdmi_v_fmt_m03x[mode].frame.interlaced, packet->gcp);
	}
// #else
#endif
...
#ifdef CONFIG_HDMI_14A_3D
	if(soc_is_exynos4210()) {
		s5p_hdmi_reg_infoframe_m030(&packet->avi_info, packet->avi);
		s5p_hdmi_reg_infoframe_m030(&packet->aui_info, packet->aui);
		s5p_hdmi_reg_infoframe_m030(&packet->spd_info, packet->spd);
		s5p_hdmi_reg_infoframe_m030(&packet->mpg_info, packet->mpg);
	} else {
		...
		s5p_hdmi_reg_infoframe_m03x(&packet->vsi_info, packet->vsi, type3D);
		s5p_hdmi_reg_infoframe_m03x(&packet->vsi_info, packet->vsi, type3D);
		s5p_hdmi_reg_infoframe_m03x(&packet->avi_info, packet->avi, type3D);
		s5p_hdmi_reg_infoframe_m03x(&packet->aui_info, packet->aui, type3D);
		s5p_hdmi_reg_infoframe_m03x(&packet->spd_info, packet->spd, type3D);
		s5p_hdmi_reg_infoframe_m03x(&packet->mpg_info, packet->mpg, type3D);
	}
// #else
#endif
...
#ifdef CONFIG_HDMI_14A_3D
	if(soc_is_exynos4210()) {
		s5p_hdmi_reg_tg_m030(&s5p_hdmi_v_fmt_m030[mode].frame);
	} else {
		s5p_hdmi_reg_tg_m03x(&s5p_hdmi_v_fmt_m03x[mode]);
	}
// #else
#endif
	if(soc_is_exynos4210()) {
		s5p_hdmi_reg_v_timing_m030(&s5p_hdmi_v_fmt_m030[mode]);
	} else {
		s5p_hdmi_reg_v_timing_m03x(&s5p_hdmi_v_fmt_m03x[mode]);
	}
...
void s5p_hdmi_ctrl_clock(bool on)
{
	...
	tvout_dbg("on(%d)\n", on);
	if (on) {
		ret = clk_enable(clk[HDMI_MUX].ptr);
		if (ret != 0) {
			printk("clk_enable failed\n");
		}
		...
		ret = clk_enable(clk[HDMI_PCLK].ptr);
		if (ret != 0) {
			printk("clk_enable failed\n");
		}
	} else {
	...
	enum s5p_tvout_disp_mode disp, enum s5p_tvout_o_mode out)
{
	...
	if(soc_is_exynos4210()) {
		if (s5p_hdmi_phy_config(frame_m030.pixel_clock, ctrl->video.depth) < 0) {
			tvout_err("hdmi phy configuration failed.\n");
			goto err_on_s5p_hdmi_start;
		}
	} else {
		if (s5p_hdmi_phy_config(frame_m03x.pixel_clock, ctrl->video.depth) < 0) {
			tvout_err("hdmi phy configuration failed.\n");
			goto err_on_s5p_hdmi_start;
		}
	}
...
void __maybe_unused s5p_tvif_ctrl_suspend(void)
{
