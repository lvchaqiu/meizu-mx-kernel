...
struct s5p_platform_tvout *s5ptv_data;
...
static DEFINE_SEMAPHORE(s5p_tvout_mutex);
					...
					struct s5p_tvout_status *ctrl)
{
	...
	TV_CLK_GET_WITH_ERR_CHECK(ctrl->i2c_phy_clk, pdev, "i2c");
...
{
	struct s5p_platform_tvout *tv_data = pdev->dev.platform_data;
