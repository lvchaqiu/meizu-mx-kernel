...
// #else
static const u8 phy_config_m030[][3][32] = {
	{ /* freq = 25.200 MHz */
		{
...
static __maybe_unused void s5p_hdmi_reg_core_reset(void)
{
	writeb(0x0, hdmi_base + S5P_E4210_HDMI_CORE_RSTOUT);

	mdelay(10);

	writeb(0x1, hdmi_base + S5P_E4210_HDMI_CORE_RSTOUT);
}
// #endif
...
static void s5p_hdmi_print_phy_config(void)
{
	...
	if(soc_is_exynos4210()) {
		size = sizeof(phy_config_m030[0][0])
			/ sizeof(phy_config_m030[0][0][0]);
	} else {
		size = sizeof(phy_config_m03x[0][0])
			/ sizeof(phy_config_m03x[0][0][0]);
	}
		...
		enum s5p_tvout_audio_codec_type audio_codec)
{
	...
	if(soc_is_exynos4210()) {
		writeb(S5P_HDMI_AUDIO_SPDIF_CLK, hdmi_base + E4210_S5P_HDMI_AUDIO_CLKSEL);
	} else {
		writeb(S5P_HDMI_AUDIO_SPDIF_CLK, hdmi_base + E4412_S5P_HDMI_AUDIO_CLKSEL);
	}
...
void s5p_hdmi_reg_sw_reset(void)
{
	...
	if (soc_is_exynos4210()) {
		writeb(0x1, hdmi_base + E4210_S5P_HDMI_PHY_RSTOUT);
		mdelay(10);
		writeb(0x0, hdmi_base + E4210_S5P_HDMI_PHY_RSTOUT);
	} else {
		writeb(0x1, hdmi_base + E4412_S5P_HDMI_PHY_RSTOUT);
		mdelay(10);
		writeb(0x0, hdmi_base + E4412_S5P_HDMI_PHY_RSTOUT);
	}
...
int s5p_hdmi_phy_power(bool on)
{
	...
	if(soc_is_exynos4210()) {
		buffer = (u8 *) phy_config_m030[0][0];
	} else {
		buffer = (u8 *) phy_config_m03x[0][0];
	}

	tvout_dbg("(on:%d)\n", on);
	if (on) {
		if (!s5p_hdmi_phy_is_enable()) {
			if(s5ptv_data && s5ptv_data->enable_power)
				s5ptv_data->enable_power(1);
		...
		enum phy_freq freq, enum s5p_hdmi_color_depth cd)
{
	...
	if(soc_is_exynos4210()) {
		size = sizeof(phy_config_m030[freq][index])
			/ sizeof(phy_config_m030[freq][index][0]);
	} else {
		size = sizeof(phy_config_m03x[freq][index])
			/ sizeof(phy_config_m03x[freq][index][0]);
	}

	if(soc_is_exynos4210()) {
		memcpy(buffer, phy_config_m030[freq][index], sizeof(buffer));
	} else {
		memcpy(buffer, phy_config_m03x[freq][index], sizeof(buffer));
	}
...
void s5p_hdmi_reg_acr(u8 *acr)
{
	...
	if (soc_is_exynos4210()) {
		hdmi_write_24(n, hdmi_base + E4210_S5P_HDMI_ACR_N0);
		hdmi_write_24(cts, hdmi_base + E4210_S5P_HDMI_ACR_MCTS0);
		writeb(4, hdmi_base + E4210_S5P_HDMI_ACR_CON);
	} else {
		hdmi_write_24(n, hdmi_base + E4412_S5P_HDMI_ACR_N0);
		hdmi_write_24(cts, hdmi_base + E4412_S5P_HDMI_ACR_MCTS0);
		writeb(4, hdmi_base + E4412_S5P_HDMI_ACR_CON);
	}
}
...
void s5p_hdmi_reg_asp(u8 *asp, struct s5p_hdmi_audio *audio)
{
	if (audio->channel == 2) {
		if (soc_is_exynos4210()) {
			writeb(S5P_HDMI_AUD_NO_DST_DOUBLE | S5P_HDMI_AUD_TYPE_SAMPLE |
				...
				hdmi_base + E4210_S5P_HDMI_ASP_CON);
		} else {
			writeb(S5P_HDMI_AUD_NO_DST_DOUBLE | S5P_HDMI_AUD_TYPE_SAMPLE |
				S5P_HDMI_AUD_MODE_TWO_CH | S5P_HDMI_AUD_SP_ALL_DIS,
				hdmi_base + E4412_S5P_HDMI_ASP_CON);
		}
	} else {
		if (soc_is_exynos4210()) {
	...
	if (soc_is_exynos4210()) {
		writeb(S5P_HDMI_ASP_SP_FLAT_AUD_SAMPLE,
			hdmi_base + E4210_S5P_HDMI_ASP_SP_FLAT);
	} else {
		writeb(S5P_HDMI_ASP_SP_FLAT_AUD_SAMPLE,
			hdmi_base + E4412_S5P_HDMI_ASP_SP_FLAT);
	}

	if (audio->channel == 2) {
		if (soc_is_exynos4210()) {
			writeb(S5P_HDMI_SPK0R_SEL_I_PCM0R | S5P_HDMI_SPK0L_SEL_I_PCM0L,
					hdmi_base + E4210_S5P_HDMI_ASP_CHCFG0);
			writeb(S5P_HDMI_SPK0R_SEL_I_PCM0R | S5P_HDMI_SPK0L_SEL_I_PCM0L,
					hdmi_base + E4210_S5P_HDMI_ASP_CHCFG1);
			writeb(S5P_HDMI_SPK0R_SEL_I_PCM0R | S5P_HDMI_SPK0L_SEL_I_PCM0L,
					hdmi_base + E4210_S5P_HDMI_ASP_CHCFG2);
			writeb(S5P_HDMI_SPK0R_SEL_I_PCM0R | S5P_HDMI_SPK0L_SEL_I_PCM0L,
					hdmi_base + E4412_S5P_HDMI_ASP_CHCFG3);
		} else {
			writeb(S5P_HDMI_SPK0R_SEL_I_PCM0R | S5P_HDMI_SPK0L_SEL_I_PCM0L,
					hdmi_base + E4210_S5P_HDMI_ASP_CHCFG0);
			writeb(S5P_HDMI_SPK0R_SEL_I_PCM0R | S5P_HDMI_SPK0L_SEL_I_PCM0L,
					hdmi_base + E4412_S5P_HDMI_ASP_CHCFG1);
			writeb(S5P_HDMI_SPK0R_SEL_I_PCM0R | S5P_HDMI_SPK0L_SEL_I_PCM0L,
					hdmi_base + E4412_S5P_HDMI_ASP_CHCFG2);
			writeb(S5P_HDMI_SPK0R_SEL_I_PCM0R | S5P_HDMI_SPK0L_SEL_I_PCM0L,
					hdmi_base + E4412_S5P_HDMI_ASP_CHCFG3);
		}
	} else {
		if (soc_is_exynos4210()) {
			...
			writeb(S5P_HDMI_SPK0R_SEL_I_PCM0R | S5P_HDMI_SPK0L_SEL_I_PCM0L,
					hdmi_base + E4412_S5P_HDMI_ASP_CHCFG0);
			writeb(S5P_HDMI_SPK0R_SEL_I_PCM1L | S5P_HDMI_SPK0L_SEL_I_PCM1R,
					hdmi_base + E4412_S5P_HDMI_ASP_CHCFG1);
			writeb(S5P_HDMI_SPK0R_SEL_I_PCM2R | S5P_HDMI_SPK0L_SEL_I_PCM2L,
					hdmi_base + E4412_S5P_HDMI_ASP_CHCFG2);
		...
		if (soc_is_exynos4210()) {
			writeb(S5P_HDMI_GCP_CON_NO_TRAN, hdmi_base + E4210_S5P_HDMI_GCP_CON);
			writeb(S5P_HDMI_SPK0R_SEL_I_PCM3R | S5P_HDMI_SPK0L_SEL_I_PCM3L,
					hdmi_base + E4210_S5P_HDMI_ASP_CHCFG3);
		} else {
			writeb(S5P_HDMI_GCP_CON_NO_TRAN, hdmi_base + E4412_S5P_HDMI_GCP_CON);
			writeb(S5P_HDMI_SPK0R_SEL_I_PCM3R | S5P_HDMI_SPK0L_SEL_I_PCM3L,
					hdmi_base + E4412_S5P_HDMI_ASP_CHCFG3);
		}
	}
...
void s5p_hdmi_reg_gcp(u8 i_p, u8 *gcp)
{
	...
	if (soc_is_exynos4210()) {
		writeb(gcp[2], hdmi_base + E4210_S5P_HDMI_GCP_BYTE2);
		gcp_con = readb(hdmi_base + E4210_S5P_HDMI_GCP_CON);
	} else {
		writeb(gcp[2], hdmi_base + E4412_S5P_HDMI_GCP_BYTE2);
		gcp_con = readb(hdmi_base + E4412_S5P_HDMI_GCP_CON);
	}
	...
	if (soc_is_exynos4210()) {
		writeb(gcp_con, hdmi_base + E4210_S5P_HDMI_GCP_CON);
	} else {
		writeb(gcp_con, hdmi_base + E4412_S5P_HDMI_GCP_CON);
	}
...
{
	if (soc_is_exynos4210()) {
		writeb(header[1], hdmi_base + E4210_S5P_HDMI_ACP_TYPE);
	} else {
		writeb(header[1], hdmi_base + E4412_S5P_HDMI_ACP_TYPE);
	}
}
...
void s5p_hdmi_reg_infoframe_m03x(struct s5p_hdmi_infoframe *info, u8 *data, u8 type_3D)
{
	...
	case HDMI_AVI_INFO:
		...
		if (soc_is_exynos4210()) {
			sum_addr	= E4210_S5P_HDMI_AVI_CHECK_SUM;
		} else {
			sum_addr	= E4412_S5P_HDMI_AVI_CHECK_SUM;
		}
	...
	case HDMI_AUI_INFO:
		...
		if (soc_is_exynos4210()) {
			sum_addr	= E4210_S5P_HDMI_AUI_CHECK_SUM;
			start_addr	= E4210_S5P_HDMI_AUI_BYTE1;
		} else {
			sum_addr	= E4412_S5P_HDMI_AUI_CHECK_SUM;
			start_addr	= E4412_S5P_HDMI_AUI_BYTE1;
		}

		break;
	case HDMI_MPG_INFO:
		if (soc_is_exynos4210()) {
			sum_addr	= E4412_S5P_HDMI_MPG_CHECK_SUM;
		} else {
			sum_addr	= E4210_S5P_HDMI_MPG_CHECK_SUM;
		}
...
void s5p_hdmi_reg_tg_m03x(struct s5p_hdmi_v_format_m03x *v)
{
	u8 tg;
	struct s5p_hdmi_v_frame_m03x	*frame = &(v->frame);
...
void s5p_hdmi_reg_v_timing_m03x(struct s5p_hdmi_v_format_m03x *v)
{
	u32 uTemp32;
...
void s5p_hdmi_reg_bluescreen_clr_m03x(u16 b, u16 g, u16 r)
{
...
// #else
void s5p_hdmi_reg_infoframe_m030(struct s5p_hdmi_infoframe *info, u8 *data)
{
	u32 start_addr = 0, sum_addr = 0;
	u8 sum;
...
void s5p_hdmi_reg_tg_m030(struct s5p_hdmi_v_frame_m030 *frame)
{
	u16 reg;
	u8 tg;
...
void s5p_hdmi_reg_v_timing_m030(struct s5p_hdmi_v_format_m030 *v)
{
	u32 reg32;

	struct s5p_hdmi_v_frame_m030	*frame = &(v->frame);
...
void s5p_hdmi_reg_bluescreen_clr_m030(u8 cb_b, u8 y_g, u8 cr_r)
{
...
void s5p_hdmi_reg_audio_enable(u8 en)
{
	...
	if (en) {
		...
		con |= S5P_HDMI_ASP_EN;
		if (soc_is_exynos4210()) {
			writeb(HDMI_TRANS_EVERY_SYNC, hdmi_base + E4210_S5P_HDMI_AUI_CON);
		} else {
			writeb(HDMI_TRANS_EVERY_SYNC, hdmi_base + E4412_S5P_HDMI_AUI_CON);
		}
	} else {
		con &= ~S5P_HDMI_ASP_EN;
		if (soc_is_exynos4210()) {
			writeb(HDMI_DO_NOT_TANS, hdmi_base + E4210_S5P_HDMI_AUI_CON);
		} else {
			writeb(HDMI_DO_NOT_TANS, hdmi_base + E4412_S5P_HDMI_AUI_CON);
		}
	}
...
void s5p_hdmi_reg_packet_trans(struct s5p_hdmi_o_trans *trans)
{
	...
	if (soc_is_exynos4210()) {
		writeb(trans->avi, hdmi_base + E4210_S5P_HDMI_AVI_CON);
		writeb(trans->aui, hdmi_base + E4210_S5P_HDMI_AUI_CON);
		writeb(trans->mpg, hdmi_base + E4210_S5P_HDMI_MPG_CON);
		writeb(trans->spd, hdmi_base + E4210_S5P_HDMI_SPD_CON);
		writeb(trans->gmp, hdmi_base + E4210_S5P_HDMI_GAMUT_CON);
	} else {
		writeb(trans->avi, hdmi_base + E4412_S5P_HDMI_AVI_CON);
		writeb(trans->aui, hdmi_base + E4412_S5P_HDMI_AUI_CON);
		writeb(trans->mpg, hdmi_base + E4412_S5P_HDMI_MPG_CON);
		writeb(trans->spd, hdmi_base + E4412_S5P_HDMI_SPD_CON);
		writeb(trans->gmp, hdmi_base + E4412_S5P_HDMI_GAMUT_CON);
	}

	reg = trans->gcp | readb(hdmi_base + E4210_S5P_HDMI_GCP_CON);
	writeb(reg, hdmi_base + E4210_S5P_HDMI_GCP_CON);

	if (soc_is_exynos4210()) {
		reg = trans->gcp | readb(hdmi_base + E4210_S5P_HDMI_GCP_CON);
		writeb(reg, hdmi_base + E4210_S5P_HDMI_GCP_CON);
		reg = trans->isrc | readb(hdmi_base + E4210_S5P_HDMI_ISRC_CON);
		writeb(reg, hdmi_base + E4210_S5P_HDMI_ISRC_CON);

		reg = trans->acp | readb(hdmi_base + E4210_S5P_HDMI_ACP_CON);
		writeb(reg, hdmi_base + E4210_S5P_HDMI_ACP_CON);
		reg = trans->acr | readb(hdmi_base + E4210_S5P_HDMI_ACP_CON);
		writeb(reg, hdmi_base + E4210_S5P_HDMI_ACR_CON);
	} else {
		reg = trans->gcp | readb(hdmi_base + E4412_S5P_HDMI_GCP_CON);
		writeb(reg, hdmi_base + E4412_S5P_HDMI_GCP_CON);
		reg = trans->isrc | readb(hdmi_base + E4412_S5P_HDMI_ISRC_CON);
		writeb(reg, hdmi_base + E4412_S5P_HDMI_ISRC_CON);
		reg = trans->acp | readb(hdmi_base + E4412_S5P_HDMI_ACP_CON);
		writeb(reg, hdmi_base + E4412_S5P_HDMI_ACP_CON);
		reg = trans->acr | readb(hdmi_base + E4412_S5P_HDMI_ACP_CON);
		writeb(reg, hdmi_base + E4412_S5P_HDMI_ACR_CON);
	}
}
