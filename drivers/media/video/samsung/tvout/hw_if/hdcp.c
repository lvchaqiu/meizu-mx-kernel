		...
		if ((zero == 20) && (one == 20)) {
			if (soc_is_exynos4210()) {
				hdmi_write_l(bksv, hdmi_base,
					E4210_S5P_HDMI_HDCP_BKSV_0_0, BKSV_SZ);
			} else {
				hdmi_write_l(bksv, hdmi_base,
					E4412_S5P_HDMI_HDCP_BKSV_0_0, BKSV_SZ);
			}
			break;
		}
	...
	if ((ri[0] == rj[0]) && (ri[1] == rj[1]) && (ri[0] | ri[1])) {
		if (soc_is_exynos4210()) {
			writeb(S5P_HDMI_HDCP_Ri_MATCH_RESULT_Y,
				hdmi_base + E4210_S5P_HDMI_HDCP_CHECK_RESULT);
		} else {
			writeb(S5P_HDMI_HDCP_Ri_MATCH_RESULT_Y,
				hdmi_base + E4412_S5P_HDMI_HDCP_CHECK_RESULT);
		}
...
static void s5p_hdcp_reset_auth(void)
{
	...
	if (soc_is_exynos4210()) {
		writeb(0x0, hdmi_base + E4210_S5P_HDMI_HDCP_CTRL1);
		writeb(0x0, hdmi_base + E4210_S5P_HDMI_HDCP_CTRL2);
	} else {
		writeb(0x0, hdmi_base + E4412_S5P_HDMI_HDCP_CTRL1);
		writeb(0x0, hdmi_base + E4412_S5P_HDMI_HDCP_CTRL2);
	}
	...
	writeb(reg, hdmi_base + S5P_HDMI_STATUS_EN);
	if (soc_is_exynos4210()) {
		writeb(S5P_HDMI_HDCP_CP_DESIRED_EN, hdmi_base + E4210_S5P_HDMI_HDCP_CTRL1);
	} else {
		writeb(S5P_HDMI_HDCP_CP_DESIRED_EN, hdmi_base + E4412_S5P_HDMI_HDCP_CTRL1);
	}
...
static int s5p_hdmi_check_repeater(void)
{
	...
	if (dev_cnt) {
		...
		do {
			if (soc_is_exynos4210()) {
				hdmi_write_l(&ksv_list[cnt*5], hdmi_base,
					E4210_S5P_HDMI_HDCP_RX_KSV_0_0, HDCP_KSV_SIZE);
			} else {
				hdmi_write_l(&ksv_list[cnt*5], hdmi_base,
					E4412_S5P_HDMI_HDCP_RX_KSV_0_0, HDCP_KSV_SIZE);
			}
			...
			if (soc_is_exynos4210()) {
				writeb(reg, hdmi_base + E4210_S5P_HDMI_HDCP_KSV_LIST_CON);
			} else {
				writeb(reg, hdmi_base + E4412_S5P_HDMI_HDCP_KSV_LIST_CON);
			}
			...
			if (cnt < dev_cnt - 1) {
				...
				do {
					if (soc_is_exynos4210()) {
						reg = readb(hdmi_base
							+ E4210_S5P_HDMI_HDCP_KSV_LIST_CON);
					} else {
						reg = readb(hdmi_base
							+ E4412_S5P_HDMI_HDCP_KSV_LIST_CON);
					}
		...
		if (soc_is_exynos4210()) {
			writeb(S5P_HDMI_HDCP_KSV_LIST_EMPTY,
				hdmi_base + E4210_S5P_HDMI_HDCP_KSV_LIST_CON);
		} else {
			writeb(S5P_HDMI_HDCP_KSV_LIST_EMPTY,
				hdmi_base + E4412_S5P_HDMI_HDCP_KSV_LIST_CON);
		}
	}
	...
	if (soc_is_exynos4210()) {
		hdmi_write_l(rx_v, hdmi_base, E4210_S5P_HDMI_HDCP_RX_SHA1_0_0,
			SHA_1_HASH_SIZE);
		reg = readb(hdmi_base + E4210_S5P_HDMI_HDCP_SHA_RESULT);
	} else {
		hdmi_write_l(rx_v, hdmi_base, E4412_S5P_HDMI_HDCP_RX_SHA1_0_0,
			SHA_1_HASH_SIZE);
		reg = readb(hdmi_base + E4412_S5P_HDMI_HDCP_SHA_RESULT);
	}
	...
	if (reg & S5P_HDMI_HDCP_SHA_VALID_RD) {
		if (reg & S5P_HDMI_HDCP_SHA_VALID) {
			...
			if (soc_is_exynos4210()) {
				writeb(0x0, hdmi_base + E4210_S5P_HDMI_HDCP_SHA_RESULT);
			} else {
				writeb(0x0, hdmi_base + E4412_S5P_HDMI_HDCP_SHA_RESULT);
			}
		} else {
...
int s5p_hdcp_start(void)
{
	...
	if (soc_is_exynos4210()) {
		writeb(sfr_val, hdmi_base + E4210_S5P_HDMI_HDCP_CTRL1);
	} else {
		writeb(sfr_val, hdmi_base + E4412_S5P_HDMI_HDCP_CTRL1);
	}
...
static int s5p_hdcp_write_aksv(void)
{
	...
	if (soc_is_exynos4210()) {
		if (s5p_hdcp_write_key(AKSV_SZ, E4210_S5P_HDMI_HDCP_AKSV_0_0, HDCP_Aksv) < 0)
			goto aksv_write_err;
	} else {
		if (s5p_hdcp_write_key(AKSV_SZ, E4412_S5P_HDMI_HDCP_AKSV_0_0, HDCP_Aksv) < 0)
			goto aksv_write_err;
	}
...
int s5p_hdcp_encrypt_stop(bool on)
{
	...
	if (hdcp_info.hdcp_enable) {
		...
		if (!sw_reset) {
			if (soc_is_exynos4210()) {
				reg = readb(hdmi_base + E4210_S5P_HDMI_HDCP_CTRL1);
			} else {
				reg = readb(hdmi_base + E4412_S5P_HDMI_HDCP_CTRL1);
			...
			if (on) {
				if (soc_is_exynos4210()) {
					writeb(reg | S5P_HDMI_HDCP_CP_DESIRED_EN,
						hdmi_base + E4210_S5P_HDMI_HDCP_CTRL1);
				} else {
					writeb(reg | S5P_HDMI_HDCP_CP_DESIRED_EN,
						hdmi_base + E4412_S5P_HDMI_HDCP_CTRL1);
				}
				s5p_hdmi_reg_intc_enable(HDMI_IRQ_HDCP, 1);
			} else {
				hdcp_info.event	= HDCP_EVENT_STOP;
