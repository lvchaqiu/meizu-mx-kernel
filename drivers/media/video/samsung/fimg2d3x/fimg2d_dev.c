...
#define BUS_LOCK_FREQ	267000
#define BUS_LOCK_TIME	50 // MSEC_PER_SEC
...
static int g2d_open(struct inode *inode, struct file *file)
{
	...
	if(atomic_read(&g2d_dev->num_of_object) == 1){
...
static int g2d_release(struct inode *inode, struct file *file)
{
	...
	if(atomic_read(&g2d_dev->num_of_object) == 0){
...
static long g2d_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case G2D_DMA_CACHE_FLUSH :
		...
		g2d_mem_cache_op(current->mm, cmd, (void *)dma_info.addr, dma_info.size);
		mutex_unlock(&g2d_dev->lock);
		return 0;
...
#ifdef CONFIG_BUSFREQ_OPP
		dev_lock_timeout(g2d_dev->bus_dev, g2d_dev->dev, BUS_LOCK_FREQ, BUS_LOCK_TIME);
#endif
...
static int g2d_probe(struct platform_device *pdev)
{
	...
	clk_set_rate(sclk, 200 * MHZ);	/* 266 Mhz */
...
#ifdef CONFIG_BUSFREQ_OPP
	g2d_dev->bus_dev = dev_get("exynos-busfreq");
	if (IS_ERR_OR_NULL(g2d_dev->bus_dev)) {
...
static void g2d_shutdown(struct platform_device *pdev)
{
	...
	while(1) {
		mutex_lock(&g2d_dev->lock);
		if (!atomic_read(&g2d_dev->in_use) || timeout<0)
		{
			mutex_unlock(&g2d_dev->lock);
			break;
		}
