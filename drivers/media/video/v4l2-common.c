...
/*
  * add by jerrymo
  * for fimc or other module to find i2c subdev by name
  */
LIST_HEAD(v4l2_i2c_subdev_list);
...
/*below 3 functions added by jerrymo, used by fimc and i2c camera*/
void add_v4l2_i2c_subdev(struct v4l2_subdev *sd)
{
	list_add(&sd->list, &v4l2_i2c_subdev_list);
}
EXPORT_SYMBOL_GPL(add_v4l2_i2c_subdev);

struct v4l2_subdev *get_v4l2_i2c_subdev(const char *name)
{
	struct v4l2_subdev *pos;

	list_for_each_entry(pos, &v4l2_i2c_subdev_list, list) {
		if (!strncmp(name, pos->name, strlen(name))) {
			pr_info("v4l2_subdev %s is found\n", __func__);
			return pos;
		}
	}
	pr_info("%s:can not find v4l2_subdev %s\n", __func__, name);
	return NULL;
}
EXPORT_SYMBOL_GPL(get_v4l2_i2c_subdev);

void del_v4l2_i2c_subdev(struct v4l2_subdev *sd)
{
	list_del(&sd->list);
}
EXPORT_SYMBOL_GPL(del_v4l2_i2c_subdev);
